...
# NEWAVE applies penalties over a fixed 730 h/month (matches network.py
# MONTH_HOURS). The flow-domain penalty coefficients in penalties.json are
# R$/(m³/s·h), so the per-stage cost of a mean flow q is q × coeff × 730.
_NEWAVE_MONTH_HOURS = 730.0

# MEDIAS-SIN cost variables already reported by NEWAVE (10⁶ R$) — used directly
# (these reuse penalties cobre-bridge converts byte-for-byte: thermal CVU from
# clast.dat, deficit cost from sistema.dat, electric-constraint penalty).
_SIN_DIRECT_COST = {
    "thermal": "CTERM",
    "deficit": "CDEF",
    "electric_violation": "CVIOL_ELETRICA",
}

# MEDIAS-USIH per-plant flow (m³/s) → penalties.json:hydro key. Cost is
# reconstructed with *our* converted penalty so the chart tests whether our
# spillage / turbined / outflow-violation penalties reproduce NEWAVE's costs.
_USIH_FLOW_COST = {
    "spillage": ("QVERTUH", "spillage_cost"),
    "turbined": ("QTURUH", "turbined_cost"),
    "outflow_violation": ("VIOL_VAZMINUH", "outflow_violation_below_cost"),
    "turbined_violation": ("VIOL_TBMIN", "turbined_violation_below_cost"),
}
...
def read_newave_stage_cost_composition(
    newave_dir: Path, penalties: dict
) -> pl.DataFrame:
    """Reconstruct NEWAVE's per-stage *live* immediate cost from MEDIAS output.

    Combines:

    - NEWAVE's own reported live cost components (CTERM thermal, CDEF deficit,
      CVIOL_ELETRICA) from MEDIAS-SIN — these use penalties cobre-bridge
      converts directly (thermal CVU, deficit cost), so they are taken as-is;
    - flow-domain costs rebuilt from MEDIAS-USIH quantities (spillage QVERTUH,
      turbined QTURUH, the min-outflow slack VIOL_VAZMINUH, …) multiplied by
      **our** converted penalties × 730 h — so the result is a direct test of
      whether our penalties reproduce the cost NEWAVE itself would charge;
    - exchange = Σ|interchange| × our exchange penalty × 730 h.

    This deliberately sidesteps the frozen-COPER artifact: it is the *live*
    immediate cost NEWAVE's dispatch implies, comparable to Cobre's per-stage
    ``immediate_cost``.

    Returns a 0-based-stage DataFrame with one column per component plus
    ``total_cost`` (all R$). Empty when the saidas dir or MEDIAS-SIN is missing.
    """
    cols = ["stage_id", *_COST_COMPONENTS, "total_cost"]
    empty = pl.DataFrame(schema={c: pl.Float64 for c in cols}).cast(
        {"stage_id": pl.Int64}
    )
    ...
    if saidas is None:
        return empty
    sin = _read_medias_csv(saidas, "MEDIAS-SIN.CSV")
    if sin.is_empty():
        return empty
    usih = _read_medias_csv(saidas, "MEDIAS-USIH.CSV")

    hydro_pen = penalties.get("hydro", {})
    line_pen = penalties.get("line", {})

    offset = int(sin["stage"].min())
    per_stage: dict[int, dict[str, float]] = {}

    def _bump(stage0: int, comp: str, value: float) -> None:
        per_stage.setdefault(stage0, dict.fromkeys(_COST_COMPONENTS, 0.0))[comp] += (
            value
        )

    # SIN direct cost components (10⁶ R$ → R$).
    for comp, var in _SIN_DIRECT_COST.items():
        for row in sin.filter(pl.col("variable") == var).iter_rows(named=True):
            if row["value"] is not None:
                _bump(int(row["stage"]) - offset, comp, float(row["value"]) * 1e6)

    # USIH flow-domain costs: Σ_plant flow × our penalty × 730 h.
    if not usih.is_empty():
        wanted = {var: (comp, key) for comp, (var, key) in _USIH_FLOW_COST.items()}
        agg = (
            usih.filter(pl.col("variable").is_in(list(wanted)))
            .group_by(["variable", "stage"])
            .agg(pl.col("value").sum().alias("q"))
        )
        for row in agg.iter_rows(named=True):
            comp, key = wanted[row["variable"]]
            coeff = hydro_pen.get(key)
            if coeff is None:
                continue
            _bump(
                int(row["stage"]) - offset,
                comp,
                float(row["q"]) * float(coeff) * _NEWAVE_MONTH_HOURS,
            )

    # Exchange (micro-penalty): Σ|interchange MW| × our exchange penalty × 730 h.
    exch_coeff = line_pen.get("exchange_cost")
    if exch_coeff is not None:
        inter = read_nwlistop_intercambio(saidas)
        if not inter.is_empty() and "value" in inter.columns:
            stage_col = "stage" if "stage" in inter.columns else "stage_id"
            ie = inter.group_by(stage_col).agg(
                pl.col("value").abs().sum().alias("flow")
            )
            ioff = int(inter[stage_col].min()) if stage_col == "stage" else 0
            for row in ie.iter_rows(named=True):
                _bump(
                    int(row[stage_col]) - ioff,
                    "exchange",
                    float(row["flow"]) * float(exch_coeff) * _NEWAVE_MONTH_HOURS,
                )
    ...
    rows = []
    for stage0 in sorted(per_stage):
        comps = per_stage[stage0]
        total = sum(comps[c] for c in _COST_COMPONENTS)
        rows.append({"stage_id": stage0, **comps, "total_cost": total})
    return pl.DataFrame(rows).select(cols).cast({"stage_id": pl.Int64})
