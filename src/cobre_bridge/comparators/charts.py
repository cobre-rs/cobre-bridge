...
def reconstructed_cost_chart(
    nw_reconstructed_costs: pl.DataFrame,
    cobre_stage_costs: pl.DataFrame,
) -> str:
    """Per-stage immediate cost: NEWAVE reconstructed (MEDIAS × our penalties) vs Cobre.

    The NEWAVE series is rebuilt from MEDIAS operation quantities multiplied by
    cobre-bridge's *converted* penalties (thermal CTERM + deficit + the
    flow-domain spillage / turbined / min-outflow-violation costs + exchange) —
    so a close match validates that our penalty conversion reproduces the costs
    NEWAVE itself charges. It is the *live* immediate cost, free of the
    frozen-COPER artifact, directly comparable to Cobre's ``immediate_cost``.
    """
    nw_by: dict[int, float] = {}
    if nw_reconstructed_costs is not None and not nw_reconstructed_costs.is_empty():
        for row in nw_reconstructed_costs.iter_rows(named=True):
            nw_by[int(row["stage_id"])] = float(row["total_cost"])

    cb_by: dict[int, float] = {}
    if cobre_stage_costs is not None and not cobre_stage_costs.is_empty():
        for row in cobre_stage_costs.iter_rows(named=True):
            v = row.get("immediate_cost")
            if v is not None:
                cb_by[int(row["stage_id"])] = float(v)

    stages = sorted(set(nw_by) | set(cb_by))
    if not stages:
        return "<p>No reconstructed cost data available.</p>"
        ...
        traces.append(
            {
                "x": stages,
                "y": _series(nw_by),
                "name": "NEWAVE reconstructed (MEDIAS × our penalties)",
                "type": "scatter",
                "mode": "lines+markers",
                "line": {"color": COLOR_NEWAVE},
                "hovertemplate": (
                    "stage %{x}<br>NEWAVE reconstructed: %{y:.2f} 10⁶ R$<extra></extra>"
                ),
            }
        )
    if cb_by:
        traces.append(
            {
                "x": stages,
                "y": _series(cb_by),
                "name": "Cobre immediate_cost",
                "type": "scatter",
                "mode": "lines+markers",
                "line": {"color": COLOR_COBRE},
                "hovertemplate": (
                    "stage %{x}<br>Cobre immediate_cost: %{y:.2f} 10⁶ R$<extra></extra>"
                ),
            }
        )

    layout = {
        "title": "Immediate Cost — NEWAVE reconstructed (from MEDIAS) vs Cobre",
        "xaxis": {"title": "Stage (0-based)"},
        "yaxis": {"title": "Cost (10⁶ R$)"},
        "legend": {"orientation": "h", "y": -0.2},
    }
