...
cobre-bridge compare newave <NEWAVE_DIR> <COBRE_OUTPUT_DIR>
...
- **Lint**: `ruff check .` + `ruff format --check .` (whole tree, mirroring the
  pre-commit hook) on Python 3.13
...
| `convert decomp` | `decomp/pipeline.py`            | Convert DECOMP case to Cobre input format        |
| `check newave`   | `preflight.py`                  | Preflight-validate inputs (no conversion)        |
| `check decomp`   | `decomp/preflight.py`           | Preflight-validate DECOMP inputs (no conversion) |
| `compare newave` | `comparators/results.py`        | Compare simulation results against Cobre         |
| `compare decomp` | `comparators/decomp_results.py` | Compare DECOMP results against Cobre             |
| `dashboard`      | `dashboard.py`                  | Generate interactive Plotly dashboard            |


`cli.py` defines a Typer `app` with sub-apps `convert`, `compare`, and
`check`, each carrying both a `newave` and a `decomp` command, plus a
...
set exit codes (convert 0/1/2, check 0/1/2, compare newave/decomp
...
src/cobre_bridge/
  ...
  decomp/                   # DECOMP conversion track (parallels converters/)
    pipeline.py             #   Conversion orchestrator (convert_decomp_case)
    preflight.py            #   `check decomp` validation engine
    fcf/                    #   Boundary FCF import (cortes, mapper, writer)
